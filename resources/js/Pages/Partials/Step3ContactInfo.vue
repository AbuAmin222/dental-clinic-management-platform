...
const props = defineProps({
  ...
  emailStatus: Object,
  checkUsernameRealTime: Function,
    ...
    <div class="m-4">
      ...
      <div class="relative mt-1">
        <TextInput
          ...
          maxlength="25"
          @input="
            form.username = form.username.replace(/[^a-zA-Z0-9_]/g, '');
            checkUsernameRealTime ? checkUsernameRealTime($event.target.value) : null;
          "
          ...
          :class="{
            ...
            'border-red-500 focus:ring-red-500':
              form.username &&
              (form.username.length < 3 || usernameStatus?.valid === false),
          }"
          placeholder="only_letters_numbers_or_underscore"
          required
        />

        <!-- أيقونات الحالة الديناميكية لاسم المستخدم -->
        <div
          class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none"
        >
          <!-- أنيميشن التحميل أثناء فحص قاعدة البيانات -->
          <svg
            v-if="usernameStatus?.loading"
          ...
          <!-- علامة الصح: الاسم متاح وغير مكرر -->
          <svg
            v-if="
              usernameStatus?.valid === true &&
          ...
          <!-- علامة الخطأ: الاسم مأخوذ مسبقاً -->
          <svg
            v-if="usernameStatus?.valid === false && !usernameStatus?.loading"
      ...
      <!-- نصوص المساعدة والتحذيرات التفاعلية سفلية الملخص -->
      <div class="flex justify-between items-center mt-1.5">
        ...
        <p
          v-else-if="usernameStatus?.message"
          :class="usernameStatus.valid ? 'text-emerald-600' : 'text-red-500'"
          class="text-xs font-semibold"
        >
          {{ usernameStatus.message }}
        </p>
        <p v-else class="text-[11px] font-medium text-slate-400">
          Allowed: A-Z, 0-9, and underscore (_). No dashes or spaces.
        </p>
        <span
